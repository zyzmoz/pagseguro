(function(){
    ...
    pagseguro = (function(){
        ...
        function session(email, token, mode, callback){
            var options = {
                method: 'POST',
                headers: {
        ...
        pagseguro.prototype.setSession = function(callback){
            this.purchase.session = session(this.email, this.token, this.email, function(err,res){
                if (err){
                    return '';
                } else {
                    return res;
            ...
            return callback(this.purchase.session != '');
        };        
        
        pagseguro.prototype.getHash = function(callback){
            console.log(this.purchase.session);
