(function(){
    ...
    pagseguro = (function(){
        ...
        pagseguro.prototype.setSender = function(obj){
            ...
            this.purchase.payment.sender.phone = { areaCode: obj.areaCode, number: obj.number } ;      
        ...
        pagseguro.prototype.checkout = function(callback){
            ...
            if (this.mode == 'deploy'){
                options.uri = envUrl.production + 'transactions?email=' + this.email + '&token='  + this.token;
            } else {
                options.uri = envUrl.sandbox + 'transactions?email=' + this.email + '&token='  + this.token;
            }
            return req(options, function(err, res, body){                
                if (err){
                    return callback(err);
